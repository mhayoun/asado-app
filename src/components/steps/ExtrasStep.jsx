...
export const ExtrasStep = ({ logic }) => {
  ...
  // Determine if RTL for icon rotation
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10 bg-white shadow-xl rounded-3xl border border-slate-100 animate-in fade-in duration-500 text-center">
      ...
      <h2 className="text-3xl font-black text-slate-900 mb-4">{t('upgrade_package', 'שדרוג חבילה')}</h2>
      <p className="text-slate-600 mb-10">{t('premium_promo', 'הפכו את הארוחה שלכם למושלמת עם החבילה המורחבת שלנו')}</p>

      {/* Toggle Section */}
      <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 mb-10 flex items-center justify-between">
        <div className="text-right flex-1">
          <h3 className="text-xl font-bold text-slate-900">{t('premium_title', 'חבילת פרימיום')}</h3>
          <p className="text-sm text-slate-500">{t('premium_desc', 'כולל שתייה קלה, סכו"ם מהודר ומשלוח חינם')}</p>
        ...
        <button
          ...
          className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors focus:outline-none ring-2 ring-offset-2 ${
