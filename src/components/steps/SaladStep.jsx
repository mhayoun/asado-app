...
export const SaladStep = ({ logic }) => {
  ...
  // 1. Create a reference for the button area
  const buttonRef = useRef(null);

  // 2. Scroll to the button when it appears
  useEffect(() => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10 bg-white shadow-xl rounded-3xl border border-slate-100 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-8 border-b pb-6">
        ...
        <div className={`text-lg font-bold p-3 px-6 rounded-full ${isComplete ? 'bg-green-100 text-green-900' : 'bg-slate-100 text-slate-700'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {initialSalads.map((salad) => (
          ...
          >
            <img src={salad.image} alt={salad.name} className="w-16 h-16 object-cover rounded-xl shadow-sm" />
      ...
      {/* 3. Wrap the button in the ref container */}
      <div className="text-center mt-12 pt-8 border-t min-h-[120px]" ref={buttonRef}>
        {isComplete && (
          <button
            onClick={() => setCurrentStep('mains')}
            className="bg-slate-900 text-white px-10 py-4 rounded-full font-bold text-lg hover:bg-black transition-all shadow-xl animate-in zoom-in duration-300 mx-auto flex items-center gap-3"
          >
            {t('next_step')}
            <ArrowRight size={18} className={i18n.language === 'he' ? 'rotate-180' : ''} />
          </button>
        )}
      </div>
    </div>
